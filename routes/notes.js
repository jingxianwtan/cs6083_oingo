...
function insertSchedule(schedule, note_id) {
  const insertScheduleQuery = `insert into schedules (note_id, start_time, end_time, start_date, end_date, frequency) 
                              values ('${note_id}', '${schedule.getStartTime()}', '${schedule.getEndTime()}', 
                              '${schedule.getStartDate()}', '${schedule.getEndDate()}', '${schedule.getFrequency()}');`;
  mysql_conn.query(insertScheduleQuery, function(err) {
    if (err) console.log(err);
  });
}
...
function getFinalNoteQuery(tagLimit, stateLimit, notesWithQuoteQuery, user) {
  ...
  if (tagLimit) {
    filter = new Filter([tagLimit], [], -1, 'everyone', user);
  } else {
    filter = new Filter(stateLimit.getTags(), stateLimit.getKeywords(), stateLimit.getWithinRadius(), stateLimit.getPostBy(), user);
  }
